...
app.controller('CampaignListController', 
  function CampaignList($scope, $routeParams,$location, Campaign) {
    var response;
    if($location.path()=='/all'){
      response = Campaign.query({}, function(data) {
        var i, l = $scope.response.results.length;
        var campaigns = $scope.response.results;
        $scope.campaignsRow = [];
        for ( i = 0; i < l; i += 3) {
          $scope.campaignsRow.push( campaigns.slice(i, i + 3) );
        }
      });
    }else{
      response = Campaign.featured({}, function(data) {
        var i, l = $scope.response.results.length;
        var campaigns = $scope.response.results;
        $scope.campaignsRow = [];
        for ( i = 0; i < l; i += 3) {
...
app.controller('CampaignDetail', 
function CampaignDetail($scope, $routeParams, Campaign) {
  var campaign = Campaign.get({id:$routeParams.id}, function() {
    $scope.master = angular.copy($scope.campaign);
    $scope.campaign_id = campaign.id;    
    $scope.cover_upload_action = "/api/campaigns/"+campaign.id+"/coverimage";
  });
  $scope.campaign = campaign;
  $scope.$emit('routeLoaded', {page: 'campaignDetail'});

  $scope.save = function(campaign) {
    Campaign.save(campaign, function(data) {
      if(data.error){
  ...
  $scope.isUnchanged = function(campaign) {
    return angular.equals(campaign, $scope.master);
  };
  ...
  $scope.doStuff = function(status) {
    if(status == 'success') {
      $scope.campaign.coverimage = $scope.coverimage.coverimage;
      $scope.master = angular.copy(campaign);
      $scope.$apply();
    }     
  }
